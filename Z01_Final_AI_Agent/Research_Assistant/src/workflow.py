...
class Workflow:
    ...
    def _build_workflow(self):
        ...
        graph.add_edge(START, "extract_urls")
        graph.add_conditional_edges("extract_urls", self.input_validation)
        graph.add_edge("gather_analyze_report", END)
        return graph.compile()
    
    # Check input for inappropriate or harmful requests
    def input_validation(self, state: ResearchState):
        """
        Check the user's query for inappropriate or harmful requests
        ...
        messages = [
            ...
            HumanMessage(content=self.prompts.input_validation_user(state.topic))
        ]
        ...
        if response.content == "True":
            ...
            return "END"
        return "gather_analyze_report"
    
    # Scrape the web for academic resources
    def find_academic_resources(self, state: ResearchState):
        """
        Find academic resources for the user's query 

        Args:
            state: The agent's saved information.

        Returns:
            Object: Contains all of the scraped urls
        """
    ...
    # Gather and then analyse scraped information 
    def gather_analyze_report(self, state: ResearchState):
        """
        Gather and then analyses scraped information to create a report

        Args:
            state: The agent's saved information.

        Returns:
            Object: The final report and scraped data

        Raises:
            Exception: If the llm fails to invoke.
        """
